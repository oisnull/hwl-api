...
namespace HWL.Redis
{
    public class CollectionStore
    {
        public class UrlModel
        {
            ...
            public bool IsValid()
            {
                return !string.IsNullOrEmpty(this.Url) && !string.IsNullOrWhiteSpace(this.Url);
            }
        }
        ...
        public static void UpdateLocalization(string rootUrl, string currentUrl, string localFileName)
        {
            ...
            RedisUtils.CollectionInstance.Exec(AppConfigManager.COLLECTION_HREFS_DB, db =>
            {
                ...
                db.HashSet(GetBeforeUrlKey(rootUrl), currentUrl, GetHashValue(val.Item1, localFileName));
            });
        }
        ...
        public static List<string> GetExistUrls(string rootUrl, List<string> urls)
        {
            ...
            RedisUtils.CollectionInstance.Exec(AppConfigManager.COLLECTION_HREFS_DB, db =>
            {
                ...
                {
                    tasks_0[i] = db.HashExistsAsync(GetBeforeUrlKey(rootUrl), urls[i]);
                    tasks_1[i] = db.HashExistsAsync(GetAfterUrlKey(rootUrl), urls[i]);
                }
                db.WaitAll(tasks_0);
                db.WaitAll(tasks_1);
        ...
        public static UrlModel GetFirstUrl(string rootUrl)
        {
            return GetUrlByIndex(rootUrl, false);
        }

        public static bool MoveUrl(string rootUrl, UrlModel urlModel)
        {
            ...
            RedisUtils.CollectionInstance.Exec(AppConfigManager.COLLECTION_HREFS_DB, db =>
            {
                ...
                {
                    succ = db.HashSet(GetAfterUrlKey(rootUrl), urlModel.Url, GetHashValue(urlModel.Level, urlModel.localFileName));
                }
            });
        ...
        public static UrlModel GetLastUrl(string rootUrl)
        {
            return GetUrlByIndex(rootUrl, true);
        }

        private static UrlModel GetUrlByIndex(string rootUrl, bool isLast)
        {
            ...
            return RedisUtils.CollectionInstance.Exec(AppConfigManager.COLLECTION_HREFS_DB, db =>
            {
                ...
                HashEntry res = db.HashScan(GetBeforeUrlKey(rootUrl), "", 1, 1, lastIndex).FirstOrDefault();
        ...
        public static List<UrlModel> GetHaveContentUrls(string rootUrl)
        {
            if (string.IsNullOrEmpty(rootUrl)) return null;
