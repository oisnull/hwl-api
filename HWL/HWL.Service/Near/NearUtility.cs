...
namespace HWL.Service.Near
{
    public class NearUtility
    {
        ...
        public static List<NearCircleCommentInfo> GetNearComments(HWLEntities db, int userId, int nearCircleId, int count = NEAR_COMMENT_PAGE_COUNT)
        {
            if (nearCircleId <= 0) return null;
            GetNearCommentsResponseBody response = new GetNearComments(db, new GetNearCommentsRequestBody()
            {
                UserId = userId,
        ...
        public static List<NearCircleLikeInfo> GetNearLikes(HWLEntities db, int userId, int nearCircleId)
        {
            if (nearCircleId <= 0) return null;
            GetNearLikesResponseBody response = new GetNearLikes(db, new GetNearLikesRequestBody()
            {
                UserId = userId,
                NearCircleId = nearCircleId,
            }).Execute();
