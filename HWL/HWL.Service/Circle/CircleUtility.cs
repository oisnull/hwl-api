...
namespace HWL.Service.Circle
{
    public class CircleUtility
    {
        ...
        public static List<CircleCommentInfo> GetComments(HWLEntities dbContext, int userId, int circleId, int count = CIRCLE_COMMENT_PAGE_COUNT)
        {
            if (circleId <= 0) return null;
            GetCircleCommentsResponseBody response = new GetCircleComments(dbContext, new GetCircleCommentsRequestBody()
            {
                UserId = userId,
                CircleId = circleId,
                LastCommentId = 0,
        ...
        //        .Select(c => new { c.id, c.content_info, c.image_count })
        ...
        //    return new Tuple<bool, List<string>>(false, infos.Select(i => i.content_info).ToList());
        //}
    }
}
